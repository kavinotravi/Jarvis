...
def test(model, test_fn, datagen, mb_size=16, conv_context=11,
    ...
    for batch in datagen.iterate_test(mb_size):
        ...
        for i, prediction in enumerate(predictions):
            pred = argmax_decode(prediction) #prediction should be fed to a ctc beam search decoder
            ler_ = ler(pred, ground_truth[i])
            print ("Truth: {}, Prediction: {}, ler : {}"
                   .format(ground_truth[i], pred, ler_))
        avg_cost += ctc_cost
        ler_sum += ler_
        i += 1
    return (avg_cost / i) , (ler_sum/i)
...
def main(test_desc_file, train_desc_file, load_dir):
    ...
    # Test the model
    test_loss, test_ler = test(model, test_fn, datagen)
    print ("Test loss: {}".format(test_loss))
    print ("Letter Error Rate: {}".format(test_ler))
