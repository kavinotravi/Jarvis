...
from utils import configure_logging, save_model, load_model, conv_output_length
...
def train(model, train_fn, val_fn, datagen, save_dir, epochs=10, mb_size=16,
    ...
    Params:
        ...
        val_fn (theano.function): A theano function that calculates the cost
            over a validation set
        output_fn: A function to get output after each epoch
        datagen (DataGenerator)
    ...
    for e in range(epochs):
        ...
        for i, batch in \
            ...
            iters += 1
            if iters % 500 == 0:
                # Checks validation costs
                # If validation set is large then validation can take a lot of time. Thus, slowing down training.
                # So to save time one can change the frequency of validation (or remove it altogether)
        ...
        if iters % 500 != 0:
            ...
            val_costs.append(val_cost)
            save_model(save_dir, model, train_costs, val_costs, iters)
        file_name = os.path.join(save_dir, "validation_output_epoch_{}.txt".format(e))
    
def main(train_desc_file, val_desc_file, epochs, save_dir, sortagrad, continue_training):
    ...
    if continue_training==True:
        ...
        # currently when restarting training, the data is read from the beginning.
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--continue_training', type= bool, default = False,
                        help='Loads the model from a previous checkpoint if continue parameter is set to True')
    args = parser.parse_args()

    main(args.train_desc_file, args.val_desc_file, args.epochs, args.save_dir,
         args.sortagrad, args.continue_training)
